...
eval env (List (Atom "if": a: b: c: [])) = do
    paran <- eval env a
    case paran of
      Bool False -> eval env (List (Atom "begin": c:[]))
      otherwise -> eval env (List (Atom "begin": b:[]))
      --data StateTransformer t = ST (StateT -> (t, StateT))
-- The following line is slightly more complex because we are addressing the
-- case where define is redefined by the user (whatever is the user's reason
-- for doing so. The problem is that redefining define does not hav
...
eval env (List (Atom "let": List bindings: body: [])) = functionLet env bindings body
eval env (List (Atom "set!": var@(Atom varStr): args)) = stateLookup env varStr >>= functionSet env var args
...
functionLet :: StateT -> [LispVal] -> LispVal -> StateTransformer LispVal
functionLet env bindings body = ST (\state -> let
    ST function = eval (localEnv (union env state) env bindings) body
    (newValue, newState) = function state
    in (newValue, state))
    
localEnv :: StateT -> StateT -> [LispVal] -> StateT
localEnv env loc [] = loc
localEnv env loc ((List ((Atom var):value:[])):vars) = localEnv env newLoc vars
  where newLoc = insert var (fst (getResult(eval env value))) loc 

--binding variavel, boddy o resto do corpo
--st mesmo do state transforme
--union do state look
--dentro do local
--casa padrao, get result devolve uma dupla, onde pegaremos o primeiro valor se for um number insere la, se for funçao n faz
--back a recursao 
...
functionSet env var args (Error t) = return (Error t)
functionSet env var args _ = define env (var:args)	
...
numericLessThan ((Number a):(Number b):[]) = Bool (a < b)
numericLessThan ((_):(_)) = Error "lt? deve receber dois números"
...
numericEquals :: [LispVal] -> LispVal
numericEquals [] = Error "wrong number of arguments."
numericEquals l = numericBoolOp (==) l
...
eqv ( (List a) : (List b) : []) = (eqvList a b)
eqv ( (Number _ ) : [(List _ )] ) = Bool False
eqv ((String a):(String b):[]) = (Bool (a==b))
eqv ((Atom a):(Atom b):[]) = (Bool (a==b))
eqv ( (List _ ) : [(Number _ )] ) = Bool False
eqv ((DottedList a b):(DottedList c d):[]) = (Bool (e && f))
  where
    (Bool e) = (eqv [b, d])
    (Bool f) = (eqv [List a, List c])
eqv ( _ : _ : _)= Error "wrong number of arguments."

eqvList [] [] = (Bool True)
eqvList [] _ = (Bool False)
eqvList _ [] = (Bool False)
eqvList (e:l) (e1:l1) = if (r) then eqvList l l1
						else (Bool False)
	where (Bool r) = (eqv (e:[e1]))
...
numericBoolOp :: (Integer -> Integer -> Bool) -> [LispVal] -> LispVal
numericBoolOp op [x] = Error "wrong number of arguments."
numericBoolOp op (x:[e]) = if (op (unpackNum x) (unpackNum e)) then Bool True 
						   else Bool False
numericBoolOp op (x:(e:l)) = if (op (unpackNum x) (unpackNum e)) then numericBoolOp op (e:l)
							   else Bool False
